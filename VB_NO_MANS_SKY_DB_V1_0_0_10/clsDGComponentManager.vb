Public Class clsDGComponentManager
    Private _TotalControls As Long
    ...
    Public Property TotalControls() As Long
        Get
            TotalControls = _TotalControls
        End Get
        Set(value As Long)
            _TotalControls = value
        End Set
    End Property
    ...
    Function AddNewPictureBox(CollectionKey As String, IsNewControl As Boolean, TheImage As Image, TheControl As Control,
        ...
        Else
            ...
            If TypeName(TheControl) = "PictureBox" Then
                ...
                NewOBJ = DirectCast(NewBox, System.Windows.Forms.PictureBox)
            Else
                frmMain.logger.LogError("NMS_ERRORS_1_0.log", Application.StartupPath, "Wrong Type - PICTUREBOX being cast to:" & TypeName(TheControl), "AddNewControl()", frmMain.GetComputerName() & "," & frmMain.GetIPv4Address() & "," & frmMain.GetIPv6Address() & ", OPERATOR LOGGED OUT:" & frmMain.myUsername)
            End If
        End If
